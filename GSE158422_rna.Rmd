...
df <- data.frame(SAMPLESIZES,dres2[,"Median"])
colnames(df) <- c("Sample size", "deseq2")

df

MAX=nrow(sig)
plot(df$`Sample size`,df$deseq2,type="b",col="black",pch=19,xlab="Sample size",ylab="no. genes", ylim=c(0,MAX))
abline(h=nrow(sig),col="black")
mtext("no. consistent genes FDR<0.05")
grid()

legend("bottomright", legend=c("deseq2"),
...
plot(df$`Sample size`,df$deseq2,type="b",col="black",pch=19,xlab="Sample size",ylab="no. pathways",ylim=c(0,MAX))
mtext("no. inconsistent genes FDR<0.05")
grid()
...
MAX=max(c(dres2[,"Median"]))
plot(df$`Sample size`,df$desseq2,type="b",col="black",pch=19,xlab="Sample size",ylab="proportion",ylim=c(0,MAX))
mtext("Proportion of inconsistent genes")
grid()

```

Now showw the pathway downsampling results.
